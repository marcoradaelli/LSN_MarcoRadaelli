...
double pdf(punto p){
	...
	double coseno2 = z*z/(r*r);
	return 1./32./M_PI*r*r*exp(-r)*coseno2; // Qui la funzione.
}
...
punto proponiPunto(punto init, double delta, Random * rnd){
	...
	fin.y=rnd->Rannyu(init.y-delta, init.y+delta);
	fin.z=rnd->Rannyu(init.z-delta, init.z+delta);
