...
// MARCO: Le funzioni da modificare sono quelle seguenti (in numero di 5).
...
double variationalWaveFunction(double v)
{
	...
	double expN = exp(- pow(v-mu,2.) /2. /pow(sigma,2.));
	double expP = exp(- pow(v+mu,2.) /2. /pow(sigma,2.));
...
double variationalWaveFunction_second(double v)
{
	...
	double expN = exp(- pow(v-mu,2.) /2. /pow(sigma,2.));
	double expP = exp(- pow(v+mu,2.) /2. /pow(sigma,2.));
	double factN = pow(v-mu, 2.)/pow(sigma, 2.) - 1./pow(sigma,2.);
	double factP = pow(v+mu, 2.)/pow(sigma, 2.) - 1./pow(sigma,2.);
	
	return factN*expN + factP*expP;
}
