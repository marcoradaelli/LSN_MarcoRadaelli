	...
	for(int i=0; i<old_1.get_quante_citta(); i++){
		...
		for(int j=0; j<dove_taglio; j++){
			if(genitore_1 -> get_citta(j) == citta_da_provare){
				flag_trovato = true;
//				cout<<"Trovato "<<citta_da_provare<<" in posizione "<<j<<endl;
			}
		}
...
//	cout<<"Completo genitore_2."<<endl;
	inserisci_qui = dove_taglio;  // Occorre re-inizializzare il punto di partenza.
